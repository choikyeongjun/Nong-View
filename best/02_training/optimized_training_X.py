...
if __name__ == "__main__":
    logger.info("=" * 80)
    logger.info("🎯 MODEL3 GREENHOUSE SEGMENTATION - YOLO11X-SEG TRAINING")
    logger.info("=" * 80)
    logger.info("Model: YOLO11X-SEG (Single Model)")
    ...
    # YOLO11X-SEG 모델만 학습
    logger.info("\n📋 Creating configuration for YOLO11X-SEG:")
    logger.info("-" * 80)
    
    config = create_training_config(
        model_type=ModelType.YOLO11X_SEG,
        dataset_type=DatasetType.MODEL3_GREENHOUSE_SEG
    )
    config.epochs = epochs
    
    # Batch size 최적화 (RTX A6000 48GB, 1024px 이미지)
    # YOLO11X-SEG: 가장 큰 모델
    config.batch_size = 16  # 1024px 기준 최적값 (메모리: ~40GB)
    # config.batch_size = -1  # 자동 조절 (필요시)
    
    logger.info(f"✓ {config.model_type.name}")
    ...
    # Confirm start
    logger.info("\n⏰ Estimated training time (1024px, RTX A6000):")
    logger.info("   - YOLO11X-SEG (1024px, batch 16): ~15 hours per 100 epochs")
    logger.info(f"\n   Current setting: {epochs} epochs → ~{epochs/100*15:.1f} hours")
    logger.info("   Expected GPU memory: 38-42GB (80-88%)")
    logger.info("\n💡 1024px 학습으로 최고 성능 달성!")
    logger.info("🚀 Starting training in 3 seconds...")
    
    import time
    time.sleep(3)
    
    # Create single model trainer
    trainer = OptimizedModelTrainer(config)
    
    # Train model
    logger.info("\n" + "=" * 80)
    logger.info("TRAINING STARTED - YOLO11X-SEG")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(f"\n📊 Final Metrics:")
        logger.info("-" * 80)
        logger.info(f"  Box mAP50      : {result['final_metrics']['mAP50']:.4f}")
        logger.info(f"  Box mAP50-95   : {result['final_metrics']['mAP50-95']:.4f}")
        logger.info(f"  Box Precision  : {result['final_metrics']['precision']:.4f}")
        logger.info(f"  Box Recall     : {result['final_metrics']['recall']:.4f}")
        
        if 'mask_mAP50' in result['final_metrics']:
            logger.info(f"\n  Mask mAP50     : {result['final_metrics']['mask_mAP50']:.4f}")
            logger.info(f"  Mask mAP50-95  : {result['final_metrics']['mask_mAP50-95']:.4f}")
            logger.info(f"  Mask Precision : {result['final_metrics']['mask_precision']:.4f}")
            logger.info(f"  Mask Recall    : {result['final_metrics']['mask_recall']:.4f}")
        
        logger.info(f"\n⏱  Training Time  : {result['training_time']:.2f} seconds")
        logger.info(f"💾 Model saved at  : {trainer.output_dir / 'best.pt'}")
        
        logger.info("\n" + "=" * 80)
        logger.info("✅ YOLO11X-SEG trained successfully!")
        logger.info("=" * 80)
